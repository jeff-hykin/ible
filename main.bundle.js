...
// https://deno.land/x/quickr@0.6.62/main/file_system.js
...
var FileSystem = {
  ...
  extname: extname3,
  join: join4,
  normalize: (path7) => normalize4(pathStandardize(path7)).replace(/\/$/, ""),
  ...
  async finalTargetOf(path7, options = {}) {
    ...
    const originalWasItem = path7 instanceof PathInfo;
    path7 = path7.path || path7;
    ...
    path7 = FileSystem.normalize(path7);
    if (originalWasItem) {
      return new PathInfo({ path: path7 });
    } else {
      return path7;
    }
  },
  async nextTargetOf(path7, options = {}) {
    const originalWasItem = path7 instanceof PathInfo;
    const item = originalWasItem ? path7 : new PathInfo({ path: path7 });
    ...
    if (lstat.isSymlink) {
      ...
      if (isAbsolute3(relativeOrAbsolutePath)) {
        ...
        if (originalWasItem) {
          return new PathInfo({ path: path8 });
        } else {
          return path8;
        }
      }
...
but existingItem didn't actually exist`);
  ...
  async *globIterator(pattern, options = { startPath: null }) {
    ...
    const regex = globToRegExp2(fullPattern);
...
// main.js
var thisFolder = FileSystem.thisFolder;
var location1 = `${FileSystem.thisFolder}/package.json`;
var location1Info = await FileSystem.info(`${FileSystem.thisFolder}/package.json`);
if (!location1Info.isFile) {
  thisFolder = `${FileSystem.thisFolder}/../ible`;
}
var packageInfo = JSON.parse(FileSystem.sync.read(`${thisFolder}/package.json`));
...
var args = (0, import_minimist.default)(realArgs, {
  default: {
    hostname: "127.0.0.1",
    port: 3e3
  },
...
var port = args.port;
var baseDirectory = path6.join(thisFolder, "docs");
var videoDirectory = path6.join(os.homedir(), "videos");
var mimeTypes = {
  ".html": "text/html",
...
var server = http.createServer((req, res) => {
  ...
  let filePath = path6.join(baseDirectory, req.url === "/" ? "index.html" : req.url);
  if (req.url.startsWith("/videos/")) {
    let videoPath = path6.join(videoDirectory, req.url.replace(/^\/videos\//, ""));
    console.debug(`videoPath is:`, videoPath);
    if (fs.existsSync(videoPath)) {
      filePath = videoPath;
    }
  }
  let extname10 = String(path6.extname(filePath)).toLowerCase();
  let contentType = mimeTypes[extname10] || "application/octet-stream";
  fs.readFile(filePath, (error, content) => {
    if (error) {
      if (error.code === "ENOENT") {
        fs.readFile(path6.join(baseDirectory, "404.html"), (err, content404) => {
