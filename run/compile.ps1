...
console.log(`    compiling ${FileSystem.makeRelativePath({from: projectFolder, to: tempMainJsPath})}`)
const targets = [
...
// alternative approach (no bundling):
    // const filesToBundle = (await FileSystem.listFilePathsIn(buildFolder)).filter(each=>each.match(/\.(js|css|html|png|jpg|jpeg|gif|ico|svg)$/))
    // 
    // let importNames
    // for (const each of filesToBundle) {
    //     const basename = FileSystem.basename(each)
    //     await binaryify({
    //         pathToBinary: eachPath,
    //         pathToBinarified: `${tempFolder}/${basename}.binaryified.js`,
    //     })
    // }
